...
function normalizeEnglishStatus(value: unknown): EnglishStatus {
  ...
  if (typeof value === "string" && VALID_ENGLISH_STATUS.includes(value as EnglishStatus)) {
...
function normalizeProfile(input: unknown): StudentProfile {
  const profile = input && typeof input === "object"
  ...
  return {
    ...
    preferredIntakeYear: normalizeString(profile.preferredIntakeYear) || String(new Date().getFullYear()),
...
function createFallbackMentorSummary(result: {
  ...
  const topMatch = Array.isArray(result.matches) && result.matches.length > 0
    ? result.matches[0] as { name?: string; provider?: string; country?: string }
    : null;
  ...
  const nextActions = Array.isArray(result.roadmap)
    ? result.roadmap
      .slice(0, 3)
      .map((step, index) => {
        const roadmapStep = step as { milestone?: string; task?: string };
        return `${index + 1}. ${roadmapStep.milestone || "Next step"}: ${roadmapStep.task || "Review your application plan."}`;
      })
      .join("\n")
...
Best scholarship direction: ${topMatch
      ? `${topMatch.name || "Top scholarship match"} from ${topMatch.provider || "the provider"} in ${topMatch.country || "your target country"}.`
...
CONTEXT:
The student is using ScholarPath AI.
- Do not invent deadlines.
- Tell them to verify official sources.
- Focus on evidence gaps and next actions.

CHAT HISTORY:
${context?.history?.map((m: { role: string; content: string }) => `${m.role.toUpperCase()}: ${m.content}`).join("\n") || ""}
...
`;
      ...
      return res.json({
        reply: response.text || "I could not generate a response. Please verify official scholarship sources and try again."
      });
    } catch (error) {
